...
public class Ranking {
	...
	public void agregarJugador(Dificultad dificultad, String nombre, int casillasDescubiertas, int minasActivadas, int tiempo) {
		...
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(archivo))) {
			for (Dificultad dif : rankings.keySet()) {
				for (Jugador j : rankings.get(dif)) {
					writer.write(j.getNombre() + "," + dif + "," + j.getCasillasDescubiertas() + "," + j.getTiempo() + "," + j.getPuntuaje());
					writer.newLine();
				}
			}
		} catch (IOException e) {
		...
		try (PrintWriter writer = new PrintWriter(new FileWriter("ranking.txt"))) {
			for (Dificultad dificultad : rankings.keySet()) {
				for (Jugador j : rankings.get(dificultad)) {
					writer.println(j.getNombre() + ";" + dificultad + ";" + j.getCasillasDescubiertas() + ";" + j.getMinasActivadas() + ";" + j.getTiempo() + ";" + j.getPuntuaje());
				}
			}
		} catch (IOException e) {
			e.printStackTrace();
